...
def main():
    ...
    def note(op, attr_nr, time):
        update_csv("charm_bsw07.csv", str(attr_nr), str(op), str(time))
    ...
    for size in range(25):
        ...
        for a in attribute_counts:
            ...
            update_csv("charm_bsw07_ct.csv", str(a), "hybrid " + str(1<<size), str(len(pickle.dumps(aes_cipher)) + len(pickle.dumps(cipher_bytes))))
